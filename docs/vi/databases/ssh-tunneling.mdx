...
<Tabs>
  ...
  <Tab title="SSH Agent">
    ...
    | Trường | Mô tả |
    |-------|-------------|
    | **Agent Socket** | Đường dẫn socket tùy chỉnh (tùy chọn) |

    Để trống để dùng `SSH_AUTH_SOCK` hệ thống.

    <Tip>
    Người dùng 1Password: đặt socket là `~/.1password/agent.sock`. Với ssh-agent macOS, để trống.
    </Tip>
  </Tab>
</Tabs>
...
| `agentSocket` | Đường dẫn socket SSH agent tùy chỉnh (ví dụ: `~/.1password/agent.sock`) |
...
2. Để trống **Agent Socket** để dùng mặc định (`SSH_AUTH_SOCK`)
3. Với 1Password, đặt socket là `~/.1password/agent.sock`
...
### Socket Agent Tùy chỉnh

| Công cụ | Đường dẫn Socket |
|------|-------------|
| 1Password | `~/.1password/agent.sock` |
| Secretive | Xem tùy chọn Secretive để lấy đường dẫn socket |
| macOS ssh-agent | Để trống (dùng `SSH_AUTH_SOCK`) |
