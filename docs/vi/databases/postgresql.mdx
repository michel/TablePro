...
<Tip>
Bạn cũng có thể kết nối trực tiếp bằng cách mở URL `postgresql://` từ trình duyệt hoặc terminal. Xem [Tham chiếu URL kết nối](/databases/connection-urls) để biết chi tiết.
</Tip>
...
<Tip>
Bạn cũng có thể kết nối trực tiếp bằng cách mở URL `postgresql://` từ trình duyệt hoặc terminal. Xem [Tham chiếu URL kết nối](/databases/connection-urls) để biết thêm chi tiết.
</Tip>
...
Bạn có thể chuyển đổi giữa các schema PostgreSQL bằng trình chuyển đổi database (**⌘K**):

1. Nhấn **⌘K** để mở trình chuyển đổi — hiển thị schemas thay vì databases cho kết nối PostgreSQL
2. Chọn schema đích (ví dụ: `auth`, `extensions`, hoặc schema tùy chỉnh)
3. Sidebar, truy vấn và toolbar sẽ cập nhật theo schema đã chọn

<Tip>
Schema mặc định là `public`. Các schema hệ thống (`pg_catalog`, `information_schema`, `pg_toast`) được lọc khỏi danh sách. Các schema bắt đầu bằng `pg_` được đánh dấu là schema hệ thống.
</Tip>

<Note>
Chuyển đổi schema thiết lập `search_path` của PostgreSQL trên kết nối. Schema đã chọn được duy trì qua các lần kết nối lại trong cùng phiên.
</Note>
