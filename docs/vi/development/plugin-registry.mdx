...
## URL Registry Tùy Chỉnh

Đối với registry plugin riêng hoặc doanh nghiệp, bạn có thể thay đổi URL registry mặc định:

```bash
defaults write com.TablePro com.TablePro.customRegistryURL "https://your-registry.example.com/plugins.json"
```

Để quay lại registry mặc định:

```bash
defaults delete com.TablePro com.TablePro.customRegistryURL
```

Ứng dụng sẽ xóa cache ETag khi URL registry thay đổi, buộc tải lại dữ liệu mới trong lần truy cập Browse tiếp theo.
