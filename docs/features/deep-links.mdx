...
TablePro checks if a saved connection matches the URL. If found, it reuses that connection. If the connection is already open, the table or query opens as a new tab in the existing window. Otherwise, a temporary connection is created.
