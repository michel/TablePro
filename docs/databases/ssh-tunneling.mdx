...
<Tabs>
  ...
  <Tab title="SSH Agent">
    ...
    | Field | Description |
    |-------|-------------|
    | **Agent Socket** | Custom socket path (optional) |

    Leave empty to use the system `SSH_AUTH_SOCK`.

    <Tip>
    1Password users: set the socket to `~/.1password/agent.sock`. For the built-in macOS ssh-agent, leave it empty.
    </Tip>
  </Tab>
</Tabs>
...
| `agentSocket` | Custom SSH agent socket path (e.g., `~/.1password/agent.sock`) |
...
2. Leave **Agent Socket** empty to use the system default (`SSH_AUTH_SOCK`)
3. For 1Password, set the socket to `~/.1password/agent.sock`
...
### Custom Agent Sockets

| Tool | Socket Path |
|------|-------------|
| 1Password | `~/.1password/agent.sock` |
| Secretive | Check Secretive preferences for the socket path |
| macOS ssh-agent | Leave empty (uses `SSH_AUTH_SOCK`) |
