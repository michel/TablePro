...
- You need to encrypt the database connection
...
For the full URL specification, see [Connection URL Reference](/databases/connection-urls#ssh-tunnel-format).
...
With SSH Agent auth, your private keys never leave the agent process — 1Password, Secretive, or the built-in macOS `ssh-agent` hold the keys and sign on behalf of TablePro.

### Setup

1. Pick **SSH Agent** as the authentication method
2. Choose an **Agent Socket** option from the dropdown
3. Select **SSH_AUTH_SOCK** to use the system default
4. Select **1Password** to use `~/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock`
5. Select **Custom Path** to enter another socket path manually

### Verify Your Agent

Confirm the agent has keys loaded:
...
### Agent Socket Options

| Option | Behavior |
|--------|----------|
| `SSH_AUTH_SOCK` | Uses the current `SSH_AUTH_SOCK` environment variable |
| `1Password` | Uses `~/Library/Group Containers/2BUA8C4S2C.com.1password/t/agent.sock` |
| `Custom Path` | Lets you enter another socket path, such as the one shown in Secretive preferences |
