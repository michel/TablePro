...
TablePro registers `postgresql`, `postgres`, `mysql`, `mariadb`, `sqlite`, `mongodb`, `redis`, `rediss`, `redshift`, and `cockroachdb` as URL schemes on macOS, so the OS routes these URLs directly to the app.
