...
description: Create, organize, and manage database connections across 11 supported engines in TablePro
---

# Connection Management

TablePro connects to eleven database systems from a single interface. Create connections, organize them with colors, tags, and groups, and switch between them without leaving your workflow.

## Supported Databases

TablePro supports eleven database systems natively:
...
Hosting platforms, deployment tools, and documentation often provide connection strings. Paste them directly into the connection form:
...
Open a database URL from your browser or terminal. TablePro launches and connects immediately, no form or saved connection required.
...
TablePro registers `postgresql`, `postgres`, `mysql`, `mariadb`, `sqlite`, `mongodb`, `mongodb+srv`, `redis`, `rediss`, `redshift`, `cockroachdb`, `mssql`, `sqlserver`, `oracle`, and `clickhouse` as URL schemes on macOS, so the OS routes these URLs directly to the app.
...
| **Type** | Database type: MySQL, MariaDB, PostgreSQL, SQLite, MongoDB, Redis, Redshift, SQL Server, or Oracle |
...
Assign colors to connections for quick visual identification:
...
When a connection has a color, the window toolbar takes on a subtle tint of that color. This gives you an at-a-glance cue of which environment you're in, similar to how Safari tints its toolbar based on website colors.
...
Tags organize connections into logical groups:
...
Groups organize connections into collapsible folders on the Welcome screen:
...
Groups collapse/expand by clicking the chevron. The collapsed state persists between sessions.
...
Switch between active or saved connections from the toolbar without returning to the Welcome screen:
...
<Tip>
The connection switcher is the fastest way to jump between databases during development. Clicking the connection button again dismisses the popover.
</Tip>
...
Faster than creating separate connections for each database on the same server.
...
Open a saved connection directly from the macOS Dock without going through the Welcome screen.

Right-click the TablePro icon in the Dock to access the Dock menu:
...
If the connection fails (e.g., server unreachable), TablePro falls back to the Welcome screen so you can review or edit the connection settings.
...
TablePro monitors active connections and auto-recovers from interruptions.

### Automatic Health Checks

For MySQL, MariaDB, and PostgreSQL connections, TablePro pings (`SELECT 1`) every **30 seconds** to verify the connection is alive. SQLite connections are file-based and skip health monitoring.

### Auto-Reconnect

When a health check detects a lost connection, TablePro reconnects automatically with exponential backoff:
...
If all three attempts fail, the connection enters an error state. A **Reconnect** button appears in the toolbar.

SSH tunnels have independent health monitoring. They are re-established automatically if the tunnel process dies.

### Manual Reconnect

If auto-reconnect fails, click the **Reconnect** button in the toolbar to re-establish the connection manually. For SSH connections, this also recreates the SSH tunnel.
...
<Warning>
Changes take effect on the next connection. If currently connected, disconnect and reconnect.
</Warning>
...
<Note>
Deleting a connection removes the saved settings and credentials. It does not affect your actual database.
</Note>
...
TablePro auto-fills the port when you select a database type:
...
<CardGroup cols={2}>
  ...
  <Card title="Redis" icon="database" href="/databases/redis">
    Redis key-value browsing and CLI
  </Card>
  <Card title="Microsoft SQL Server" icon="database" href="/databases/mssql">
    SQL Server connections via FreeTDS
  </Card>
  <Card title="Oracle Database" icon="database" href="/databases/oracle">
    Oracle Database connections via OCI
  </Card>
  ...
  <Card title="CockroachDB" icon="database" href="/databases/cockroachdb">
    CockroachDB distributed SQL connections
  </Card>
  <Card title="ClickHouse" icon="database" href="/databases/clickhouse">
    ClickHouse OLAP database connections
  </Card>
