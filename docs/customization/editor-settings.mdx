...
When enabled, pressing `Cmd+Enter` on a query with `:name` placeholders opens an inline panel for entering values. Parameters are bound via native prepared statements. See [Query Parameters](/features/query-parameters) for details.
