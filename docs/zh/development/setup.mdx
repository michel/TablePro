...
| **SwiftFormat** | 代码格式化 |
| **Homebrew** | 包管理器 |
| **GitHub CLI (`gh`)** | 从 GitHub Releases 下载预编译的原生库 |
...
<Warning>
跳过 `scripts/download-libs.sh` 会导致 `Libs/` 目录下没有二进制文件，从而引发链接器错误。
</Warning>
...
在仓库根目录运行 `scripts/download-libs.sh` 和 `touch Secrets.xcconfig`。参见[步骤 1](#步骤-1克隆和初始化)。
