...
- AI Chat "Go to Settings..." button on the "AI Not Configured" empty state did nothing. The action posted `showSettingsWindow:` via `NSApp.sendAction`, a legacy AppKit selector that does not fire on macOS 14+. Now routes through `WindowOpener.openSettings(tab: .ai)`, the same path other settings entry points use.
